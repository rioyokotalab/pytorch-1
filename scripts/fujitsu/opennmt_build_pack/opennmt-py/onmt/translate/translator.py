...
class Statistics(object):
    """
    Accumulator for time statistics.
    Currently calculates:

    * elapsed time
    """

    def __init__(self, n_words=0, n_src_words=0):
        self.n_words = n_words
        self.start_time = time.time()
        self.batch_start_time = time.time()
        self.batch_avg_time = 0
        self.token_per_sec = 0

    def update(self, n_words):
        """
    ...
    def batch_elapsed_time(self):
        """ compute batch elapsed time """
        batch_elapse = time.time() - self.batch_start_time
        self.batch_start_time = time.time()
        return batch_elapse
    ...
    def output(self, step):
        ...
        t = self.elapsed_time()
        logger.info(
            ("Step %d; %3.1f tok/s; Batch Time %6.3f sec; Total %6.3f sec")
            % (step,
               self.n_words / (t + 1e-5),
               self.batch_elapsed_time(),
               time.time() - self.start_time))
        self.batch_avg_time = t / (step + 1e-5)
        self.token_per_sec = self.n_words / (t + 1e-5)
        sys.stdout.flush()
...
class Translator(object):
    ...
    def translate(
        ...
        for step, batch in enumerate(data_iter):
            if translate_steps > 0 and step >= translate_steps:
                break
            ...
            for trans in translations:
                ...
                report_stats.update(len(trans.pred_sents[0]))
        ...
        return all_scores, all_predictions, report_stats

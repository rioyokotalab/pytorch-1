...
def multi_init(opt, device_id):
    ...
    rank = opt.gpu_ranks[device_id] if opt.cpu_rank == -1 else opt.cpu_rank
    backend = opt.gpu_backend if opt.cpu_rank == -1 else opt.cpu_backend
    torch.distributed.init_process_group(
        backend=backend, init_method=dist_init_method,
        world_size=dist_world_size, rank=rank)

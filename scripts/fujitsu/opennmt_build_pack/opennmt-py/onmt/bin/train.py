...
def train(opt):
    ...
    nb_dev = len(opt.gpu_ranks) if opt.cpu_rank == -1 else 1
    ...
    elif nb_dev == 1:  # case 1 Device only
        single_main(opt, 0)
    else:   # case only CPU
        single_main(opt, -1)
...
def batch_producer(generator_to_serve, queues, semaphore, opt):
    ...
    def pred(x):
        ...
        """
        if opt.cpu_rank != -1:
            if x[0] % opt.world_size == opt.cpu_rank:
                return True
        else:
    ...
    for device_id, q in cycle(enumerate(queues)):
        device_type = "cuda" if use_gpu(opt) else "cpu"
        b.dataset = None
        if isinstance(b.src, tuple):
            b.src = tuple([_.to(torch.device(device_type, device_id))
                           for _ in b.src])
        else:
            b.src = b.src.to(torch.device(device_type, device_id))
        b.tgt = b.tgt.to(torch.device(device_type, device_id))
        b.indices = b.indices.to(torch.device(device_type, device_id))
        b.alignment = b.alignment.to(torch.device(device_type, device_id)) \
            if hasattr(b, 'alignment') else None
        b.src_map = b.src_map.to(torch.device(device_type, device_id)) \
            if hasattr(b, 'src_map') else None
        b.align = b.align.to(torch.device(device_type, device_id)) \
            if hasattr(b, 'align') else None
        b.corpus_id = b.corpus_id.to(torch.device(device_type, device_id)) \
...
def run(opt, device_id, error_queue, batch_queue, semaphore):
    """ run process """
    try:
        dev_rank = onmt.utils.distributed.multi_init(opt, device_id)
        if opt.cpu_rank == -1:
            if dev_rank != opt.gpu_ranks[device_id]:
                raise AssertionError("An error occurred in \
                Distributed initialization on CPU")
        else:
            if dev_rank != opt.cpu_rank:
                raise AssertionError("An error occurred in \
                Distributed initialization on CPU")
        ...
        if opt.cpu_rank == -1:
            ...
            error_queue.put((opt.cpu_rank, traceback.format_exc()))
